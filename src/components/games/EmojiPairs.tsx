...
import { EMOJI_PAIRS } from '../../data/gamesLibrary'

interface EmojiPairsProps {
  onBack: () => void
}
...
export default function EmojiPairs({ onBack }: EmojiPairsProps) {
  ...
  function shuffleCards(): Card[] {
    ...
    EMOJI_PAIRS.forEach((emoji) => {
      cardArray.push(
        { id: emoji.id * 2, emojiId: emoji.id, isFlipped: false },
        { id: emoji.id * 2 + 1, emojiId: emoji.id, isFlipped: false },
      )
    })
  ...
  const handleCardClick = (cardId: number) => {
    ...
    if (newSelectedCards.length === 2) {
      ...
      if (card1.emojiId === card2.emojiId) {
        // Match found!
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-br from-yellow-300 via-orange-300 to-pink-300 flex flex-col items-center justify-center p-4">
      {/* Header */}
      ...
      <div className="grid grid-cols-3 gap-4 md:gap-5 mb-8 max-w-2xl">
        {cards.map((card) => {
          const emoji = EMOJI_PAIRS.find((e) => e.id === card.emojiId)
          return (
            <button
              key={card.id}
              onClick={() => handleCardClick(card.id)}
              className={`w-20 h-20 md:w-24 md:h-24 rounded-2xl font-bold text-4xl transition-all transform ${
                matched.includes(card.id)
                  ? 'bg-green-400 scale-110 shadow-lg'
                  : selectedCards.includes(card.id)
                    ? 'bg-purple-400 shadow-xl scale-95 animate-pulse'
