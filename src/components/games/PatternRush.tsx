...
function getRoundPattern(round: number) {
  return QUICK_PATTERNS[round % QUICK_PATTERNS.length]
}
...
export default function PatternRush({ onBack }: PatternRushProps) {
  ...
  useEffect(() => {
    if (answered || completed || timedOut) return
  ...
  }, [round, answered, completed, timedOut])

  useEffect(() => {
    if (!timedOut || answered || completed) return
  ...
  }, [timedOut, answered, completed, round])
  ...
  const handleSelect = (emoji: string) => {
    if (answered || timedOut || completed) return
    setAnswered(true)
    ...
    <div className="w-full h-full bg-gradient-to-br from-purple-300 via-indigo-300 to-violet-400 flex flex-col overflow-hidden">
      <div className="flex justify-between items-center p-4 md:p-6 bg-white/20 backdrop-blur-sm">
        ...
        <div className="text-center">
          <div className="text-2xl md:text-4xl font-bold text-white">⚡ Pattern Rush</div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-8 gap-6">
        <div className="flex items-center gap-4">
          <div className="text-white font-bold text-lg">Round {round + 1}/{TOTAL_ROUNDS}</div>
          <div
